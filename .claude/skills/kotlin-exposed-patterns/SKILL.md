...
> ⚠️ **Exposed v1.0.0+ introduced breaking package namespace changes** (`org.jetbrains.exposed` → `org.jetbrains.exposed.v1`).
> If using v1.0.0 or later, all imports must use the `v1` prefix.
> Latest stable is **1.2.0** — prefer upgrading to avoid migration pain.

```kotlin
// build.gradle.kts
dependencies {
    // Exposed (v1.0.0+ uses org.jetbrains.exposed.v1 package namespace)
    // For v1.2.0 (latest), use:
    ...
    // For v1.0.0 (deprecated, requires v1 imports):
    // implementation("org.jetbrains.exposed:exposed-core:1.0.0")
    // All imports must use org.jetbrains.exposed.v1.* prefix
...
### Import Migration (v0.61.0 → v1.0.0+)
